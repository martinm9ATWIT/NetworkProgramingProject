...
def threaded_client(connection, num):
    ...
    while True:
        ...
        try:
            mycursor.execute("SELECT PASSWORDS,WEBSITES FROM mydatabase." + str(
                clientReplyMessage) + ";")  # SQL query too get the stored passwords with the given username
            ...
            for x in str(myresult):
                if x.isalnum() or x == "(" or x == ")" or x == "," or x == ".":
                    if x == ",":
                        x = "    "  # 4 spaces
                    if x == ")":
                        x += "\n"
            ...
            if newPass != "" or newWeb != "":
                ...
                sql = "INSERT INTO mydatabase." + str(
                    clientReplyMessage) + " (PIN,PASSWORDS,WEBSITES) VALUES (%s,%s,%s)"
            ...
            mycursor.execute(
                "CREATE TABLE " + str(newUser) + " (PIN VARCHAR(5) , PASSWORDS VARCHAR(30), WEBSITES VARCHAR(30));")
            sql = "INSERT INTO mydatabase." + str(
                newUser) + " (PIN,PASSWORDS,WEBSITES) VALUES (%s,%s,%s)"  # Insert the pin into the tabel
            val = (str(newPin), '', '')
            mycursor.execute(sql, val)
            mydb.commit()
            break;
